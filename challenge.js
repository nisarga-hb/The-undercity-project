(function () {
  ...
  function initMatrixRain() {
    ...
    function resize() {
      ...
      var newCols = Math.floor(canvas.width / fs);
      if (!drops || newCols !== cols) {
        var oldDrops = drops || [];
        drops = Array(newCols).fill(1);
        for (var j = 0; j < Math.min(oldDrops.length, newCols); j++) drops[j] = oldDrops[j];
        cols = newCols;
      }
    }
